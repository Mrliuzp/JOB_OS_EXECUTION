...
POLICIES = {
    "message_policy": {
        ...
        "manual_review_types": ["salary", "interview_schedule", "technical_question", "personal_information"],
...
async def test_sync_deduplicates_and_availability_can_auto_reply(session: Session) -> None:
    ...
    fact = profile_service.add_fact(
        profile.id,
        CandidateFactInput(fact_type="availability", statement="每周可投入 15 至 20 小时，支持远程"),
    )
    account = PlatformAccountORM(provider="mock", display_name="primary", status="logged_in")
    session.add(account)
    JobService(session).import_text("Python 远程兼职开发", "示例科技", "Python FastAPI 远程兼职", "mock-job-1")
    session.flush()
    ...
    assert "发送" in detail
    assert await service.send_reply(message.id, MockProvider(), account, dry_run=False) == "幂等命中，未重复发送"
